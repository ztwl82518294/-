...
- **【已删除】「月度兑奖」活动**（2026-09-21，提交 `02d43d0`）：用户端 `pages/prize`、管理端 `pages/admin/prize`、云函数 `cloudfunctions/prize` 及全部入口/索引/协议表述已清除。小程序现为 **7 用户页 + 7 管理页**。保留 `stat_events` 统计（仅供平台自用评估，不再发奖）。
