...
数据模型为 6 张表（公司 / 线路 / 线路-公司关联 / 城市 / 纠错 / 管理员），
...
├── test/                  测试（框架 + 5 个套件 + 统一入口）
├── scripts/               工程脚本（体检 / 导出种子 / 后台冒烟）
└── docs/                  文档
```
...
然后浏览器打开 **http://127.0.0.1:8787**，默认账号 `admin` / `admin12345`（**登录后请尽快改密码**）。
...
```bash
node test/run-all.js         # 5 个套件，335 个断言
node scripts/check-project.js  # 静态体检：绑定 / dataset / 路由 / wx:key / BOM / 测试套件
node scripts/smoke-admin.js    # 后台 HTTP 冒烟，72 个断言（会临时占用 8791 端口）
```
